import { Nominatim } from "../../../../src/infra/GeocodApi/apis";
import { HttpGet, HttpGetParams } from "../../../../src/interfaces/gateway/http";
import { validLocalReceived, validLocalResponse } from "./faker-data/nominatim";
...
const makeSut = (): SutTypes => {
    ...
    const sut = new Nominatim(httpGetStub);
...
describe("Here Geocoder api", () => {
    it("Should return address formated", () => {
        const { sut } = makeSut();
        const address = "RUA ITAJUBA; 980, CASA #37 //20, CIDADE, ES";
        const formatedAddress = sut.getFormatedAddress(address);

        expect(formatedAddress).toBe("RUA+ITAJUBA,+980,+CASA+37+//20,+CIDADE,+ES");
    });
    ...
    //     expect(url).toBe(`https://nominatim.openstreetmap.org/?addressdetails=1&q=${address}&format=json&countrycodes=BR&limit=1`);
    // });
